...
import nacl.signing
from anchorpy import Idl, Program, Provider,InstructionCoder
...
provider = Provider(AsyncClient("https://api.devnet.solana.com"), None)  # No wallet needed for verification
...
async def verify_upload_transaction(tx_signature: str, expected_pdf_hash: str, expected_access_level: int, expected_document_index: int, user_public_key: str) -> bool:
    ...
    if (args.pdf_hash == expected_pdf_hash and
        args.access_level == expected_access_level and
        args.document_index == expected_document_index):
        signers = message['accountKeys'][:len(tx['result']['transaction']['signatures'])]
